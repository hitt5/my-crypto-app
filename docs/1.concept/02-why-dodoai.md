...
| dodoAI 能力 | どの課題に効くか |
|---|---|
| SDT（Semantic Digital Twin） | 課題0〜2・5: **Minimal Causal World Model**（境界変数 = ドル流動性・金利・Stablecoin 供給・Unlock 等）と **Constraint Graph**（誰が・いつ・なぜ動かざるを得ないか）・Viability 空間（生存制約・Ω・ruin 距離）を Entity/State/Relation/Constraint/Goal として構造化し、常に「今どうなっているか」を保持する。「意味を保ったままエントロピーを下げる」という dodoAI の思想は、実体経済の Digital Twin ではなく Crypto へ因果流入する境界変数だけを残す Minimal Causal 原則そのものと合致する |
| Fact / Belief / Hypothesis の分離統治 | 課題3・6: 「オンチェーン実測（Fact）」「予測分布・エアドロップ可能性（Belief, confidence 付き）」「新プロトコルの将来性（Hypothesis）」を分離し、SNS 投稿やインフルエンサー発言が事実として売買に接続される事故を構造的に排除する。単一予測値ではなく分布での保持（CR-9.3）を統治として強制する |
| Evidence = Provenance | 課題8: 全取引の Intervention（提案→検査→署名→執行）と Observation（結果）を対で Evidence Ledger に記録。予測分布と実績の対（forecast error）・Counterfactual 実績を蓄積し、**IG（情報利得）を測定可能にする**。「なぜ投資したか」「どの情報源が正しかったか」「何もしない方がよかったか」を遡れる。税務用の取得価格・取引理由の記録も兼ねる |
| Multi-Agent | 課題2〜4・7: World Model / Constraint / Crowding / Viability Agent の観測・評価を Risk Agent（不可逆性・因果相関・Fractional Kelly）と Red Agent（失敗条件の攻撃的検証）が構造的に批判する。多数決にせず、最終可否は決定論的 Policy Engine が判断する |
| HIL | 課題6・7: Observe → Approve → Autopilot の3段階で人間承認の範囲を段階的に縮小。**不可逆な Action・生存制約（ε）の変更・緊急退避・権限変更は常に人間承認**。Agent 自身に権限上限を変更させない |
| Domain Universality | Airdrop / Yield / リバランス / Constraint イベント / Guardian という異なるドメインの機会・リスクを、同一の「生存制約付き Intervention 空間」構造（状態・目標・制約・因果）で統合管理できる |

## この手段が最適である理由

このプロダクトの価値は「どの通貨が上がるか（予測精度）」ではなく、**生存原理の機械的執行と投資判断の組織知**にある。

1. **生存制約は統治機構でしか強制できないから**: 「P(ruin) < ε を維持する」「不可逆な Action は必ず人間承認」「サイズは不確実性で制限」という原則は、宣言するだけでは守られない。権限制御（Agent Governance）・決定論的 Gate・判断根拠追跡（Evidence）・人間の最終権限（HIL）を基盤に持つ dodoAI でなければ、後付けで実現できない。単なる LLM + 取引 API では API キー丸渡しのブラックボックスに退行する。
2. **秘密鍵の分離統治 = 最大の不可逆リスクの構造的排除**: 秘密鍵は AI に渡さない。鍵管理は dodo のクレデンシャル機構（DID/VC・`env://` 参照・Desktop によるユーザー環境内の署名処理）を基本とし、**個人のハードウェアモジュール（ハードウェアウォレット等）で補完する二層構成**。Agent が持つのは Session Key / Safe Module 経由の期限付き・用途限定の実行権限のみ（詳細 = [`../3.common/0.common-requirements/00-common-requirements.md`](../3.common/0.common-requirements/00-common-requirements.md) CR-1）。
3. **IG（情報利得）の蓄積装置だから**: どの予測分布が校正されていたか・どの情報源・戦略バージョンが正しかったかという投資判断の知は、Fact/Belief/Hypothesis 分離 + Evidence（予測と実績の対）の SDT にだけ蓄積できる。大原則の IG 項はこの蓄積構造なしには測定できない。
4. **「何もしない」を正当化できるから**: デフォルト不作為・No-action Counterfactual 比較・回転数上限・増分利益測定という規律は、Goal/Constraint を明示した SDT と決定論的 Gate があって初めて機械的に強制できる。
...
| バージョン | 日付 | 内容 |
|---|---|---|
| v1.3.0 | 2026-08-22 | **根本改訂に追随**: 効き所を新原理（Minimal Causal World Model・Constraint Graph・生存制約・IG・不可逆性）へ再対応付け。「意味を保ったままエントロピーを下げる」と Minimal Causal 原則の接合を明記。固定部分は変更せず |
